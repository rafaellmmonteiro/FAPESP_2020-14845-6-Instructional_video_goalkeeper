...
while(saltos<len(dir_working)):
    ...
    while (cont <= len(cmdat[:,0])-2):
        ...
        if (cont == framepe[saltos]+2 or cont == framepe[saltos]+6): #tem um salto em que só tem 4 frames entre o contato do pé e o fim do salto
    ...
    saida[0,5]=TVMLP
    saida[0,6]=tampernaf
    saida[0,7]=distcalcanharesy
    saida[0,8]=passofrontal
    saida[0,9]=angsaidafrontal
    saida[:,10]=angjoelhospm
    saida[0,11]=angjoelhomin
    
    fmt = '%6f', '%6f', '%6f', '%6f', '%6f', '%6f', '%6f', '%6f', '%6f', '%6f', '%6f', '%6f'
    np.savetxt(dir_atual + '/workingprocessado/' + saltoanalisado, saida, fmt=fmt)
    saltos = saltos+1
    print(f'{saltoanalisado} processado com sucesso')
