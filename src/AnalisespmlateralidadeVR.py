...
CALCULO DO SPM PARA VR EM RELAÇÃO A LATERALIDADE
"""
...
mediapf = np.mean(cpre[:,100])
mediapnf = np.mean(cpos[:,100])

fig1, f1_axes = plt.subplots(ncols=2, nrows=2)
fig1.suptitle('Efeito da lateralidade', fontsize=16)
...
#f1_axes.plot(radm)
plt.subplot(2,2,1)
plt.plot(cprem, label = 'Perna não dominante')        #por algum motivo inverteu os resultados, mudei os nomes para aproveitar o gráfico
...
plt.plot(cposm, label = 'Perna dominante')
plt.errorbar(t, cposm, color = 'b')
plt.fill_between(t, cposm[:,0]+cposdp[:,0], cposm[:,0]-cposdp[:,0], color = 'cornflowerblue', alpha = 0.5)
plt.xlabel('Ciclo (%)')
plt.ylabel('Velocidade do CM (m/s)', fontsize=10)
plt.title('Velocidade do CM', size='12')
...
cposdp=cposdp.T
#f1_axes.plot(radm)
plt.plot(cprem, label = 'Perna não dominante')
t = np.linspace(0,len(cprem),len(cprem), False)
plt.errorbar(t, cprem, color = 'r')
plt.fill_between(t, cprem[:,0]+cpredp[:,0], cprem[:,0]-cpredp[:,0], color = 'lightcoral', alpha=0.5)
plt.plot(cposm, label = 'Perna dominante')
...
plt.fill_between(t, cposm[:,0]+cposdp[:,0], cposm[:,0]-cposdp[:,0], color = 'cornflowerblue', alpha = 0.5)
plt.xlabel('Ciclo (%)')
plt.ylabel('Ângulho do joelho (°)', fontsize=10)
plt.title('Ângulo do joelho ipsilateral ao salto', size='12')
plt.legend(loc=2, prop={'size': 8})