...
#######################################################
t  = spm1d.stats.ttest_paired(cpre, cpos)
...
#Plotando gráfico
fig3, f3_axes = plt.subplots(ncols=1, nrows=1)
cprem=cprem.T
cpredp=cpredp.T
cposm=cposm.T
cposdp=cposdp.T
...
plt.title('Ângulo do joelho do grupo controle pré e pós')
...
#Plotando gráfico
fig2, f2_axes = plt.subplots(ncols=1, nrows=1)
vprem=vprem.T
vpredp=vpredp.T
vposm=vposm.T
vposdp=vposdp.T
...
plt.xlabel('Ciclo (%)')
plt.ylabel('Velocidade do centro de massa')
plt.title('Ângulo do joelho do grupo vídeo pré e pós')
plt.legend(loc=2, prop={'size': 10})

fig1, f1_axes = plt.subplots(ncols=2, nrows=2)
fig1.suptitle('Ângulo do joelho ipsilateral ao salto', fontsize=16)
#realizando o teste SPM
#######################################################
t  = spm1d.stats.ttest_paired(vpos, cpos)
ti = t.inference(alpha=0.05, two_tailed=False)
plt.subplot(2,2,4)
ti.plot()
spm1d.plot.plot_spmi_p_values(ti)
...
print( ti )
print( ti.clusters )
plt.xlabel('Ciclo (%)', size= '10')
plt.ylabel('SPM{t}', size= '10')
plt.title('Ciclo SPM', size='12')
#######################################################
...
#f1_axes.plot(radm)
plt.subplot(2,2,2)
plt.plot(cposm, label = 'Grupo vídeo pós')
t = np.linspace(0,len(cposm),len(cposm), False)
plt.errorbar(t, cposm, color = 'r')
plt.fill_between(t, cposm[:,0]+cposdp[:,0], cposm[:,0]-cposdp[:,0], color = 'lightcoral', alpha=0.5)
plt.plot(vposm, label = 'Grupo controle pós')
plt.errorbar(t, vposm, color = 'b')
plt.fill_between(t, vposm[:,0]+vposdp[:,0], vposm[:,0]-vposdp[:,0], color = 'cornflowerblue', alpha = 0.5)
plt.xlabel('Ciclo (%)')
plt.ylabel('Ângulo do joelho (°)', fontsize=10)
plt.title('Condição pós', size='12')
plt.legend(loc=2, prop={'size': 8})

#realizando o teste SPM
#######################################################
t  = spm1d.stats.ttest_paired(vpre, cpre)
ti = t.inference(alpha=0.05, two_tailed=False)
plt.subplot(2,2,3)
ti.plot()
spm1d.plot.plot_spmi_p_values(ti)
...
#fig1, f1_axes = plt.subplots(ncols=1, nrows=1)
#f1_axes.plot(radm)
plt.plot(cprem, label = 'Grupo vídeo pré')
t = np.linspace(0,len(cprem),len(cprem), False)
plt.errorbar(t, cprem, color = 'r')
plt.fill_between(t, cprem[:,0]+cpredp[:,0], cprem[:,0]-cpredp[:,0], color = 'lightcoral', alpha=0.5)
plt.plot(vprem, label = 'Grupo controle pré')
plt.errorbar(t, vprem, color = 'b')
plt.fill_between(t, vprem[:,0]+vpredp[:,0], vprem[:,0]-vpredp[:,0], color = 'cornflowerblue', alpha = 0.5)
plt.xlabel('Ciclo (%)')
plt.ylabel('Ângulo do joelho (°)', fontsize=10)
plt.title('Condição pré', size='12')
plt.legend(loc=2, prop={'size': 8})